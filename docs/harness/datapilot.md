...
> 本文是 DataPilot 接入、查询和本地联调手册，不是项目通用开发规范。

> 本文根据 CSES 项目当前代码整理，描述 DataPilot 在项目中的依赖引入、初始化、模型定义、数据访问、索引搜索和本地联调方式。

## Flow 项目接入说明

Flow 项目按本文模式接入 DataPilot，但当前只注册自身的 PostgreSQL 数据源：

| 配置项 | Flow 项目值 |
| --- | --- |
| 引擎键 | `flow` |
| 默认数据源键 | `postgresql.flow` |
| 数据库名 | `flow` |
| jOOQ Schema | `public` |
| 会话/用户 | `Session<User>` / `User` |
...
Elasticsearch 和 Redis 等物理连接仍由部署环境或 Consul 提供，不能写入 Java
源码。
