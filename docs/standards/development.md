...
- Service、Handler、Domain 和 Repository 之间传递同一个 ID，不得由不同层重复
  生成。
- 已接受 ADR 明确采用追加式版本记录时，每次追加的是一条新的持久化记录，可以在
  该 ADR 指定的持久化边界为新行生成一次技术 ID；这不允许普通更新重新生成 ID。
...
- 业务标识由业务规则或外部输入决定，不能代替技术 ID，也不能使用技术 ID 生成器
  代替业务规则。
- 查询、更新和恢复已有持久化行时必须校验并使用已有 ID，不得生成新 ID；追加式
  版本保存按对应 ADR 生成新行 ID。
...
- 方法级 Javadoc 的说明文字统一使用中文，包括正文以及 `@param`、`@return`、
  `@throws` 的说明；Javadoc 标签、代码标识符和必要的技术专有名词保留原文。
