...
结构，并与已确认的直接 `resume`、`duration`、`behavior` 属性冲突。

### 方案三：Pause 直接拥有专有字段

具体类直接声明 `pause`、`resume`、`duration` 和 `behavior`。Task 基类只补充定义
...
- `Pause` 直接拥有：
  - `Task pause`：必填且唯一，表示进入 `PAUSED` 前必须完整执行的 Task；允许任意
    RunnableTask 或 OrchestrationTask 及其完整子树。
  - `List<Input<?>> resume`：外部 Resume 回调的数据定义；非 null，允许为空。
...
- 超时触发 RESUME 时使用空回调数据进入同一 Input 校验；校验失败的目标结果是
  FAILED。WARN、CANCEL、FAIL 不反向推导或填充 resume 输入。
...
- 变异：若 pause 未执行就进入 PAUSED、若 Resume 直接变为 COMPLETED、若
  Execution 随 Pause 进入 PAUSED，测试必须失败。
- 恢复：重启后从持久化 Flow Reversion 找到嵌入的 pause 定义，并以同一
...
- 真实 Pause 类同时成为类型、字段、Schema 与属性序列化的唯一事实来源，符合
  Task 即 Plugin 的扩展边界。
- 把 pause 与通用 tasks 分开后，定义能够准确表达动作发生在进入等待之前。
