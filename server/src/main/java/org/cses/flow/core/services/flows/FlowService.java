...
public final class FlowService {
    ...
    FlowDraft saveDraft(S session, String raw) {
        return commandExecutor.execute(
                session,
                new SaveFlowDraftCommand(null, null, raw)
        );
    }
    ...
    public <S extends Session<U>, U extends User> FlowDraft saveDraft(
        ...
        FlowDraft existing = draft(session, id).orElseThrow(() ->
                new WorkflowException(
                        "FlowDraft does not exist: " + id
                )
        );
        return saveDraft(
                session,
    ...
    public <S extends Session<U>, U extends User>
    FlowDraft deleteDraft(S session, String id) {

        return commandExecutor.execute(
                session,
                new DeleteFlowDraftCommand(id)
        );
    }

    public <S extends Session<U>, U extends User>
    Flow delete(S session, String id) {

        return commandExecutor.execute(
                session,
                new DeleteFlowCommand(id)
        );
    }
