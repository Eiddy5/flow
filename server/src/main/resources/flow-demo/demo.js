(() => {
    ...
    function renderBootError(error) {
        document.querySelector("#app").innerHTML = `
            <div class="boot-screen">
                ...
                <strong>无法连接 Flow Demo API</strong>
    ...
    function renderRail() {
        const initials = String(state.session?.userName || "Demo")
            .split(/\s+/)
            .map((part) => part[0])
            .join("")
            .slice(0, 2)
        ...
        return `
            <aside class="rail">
                ...
                <button class="rail-button" title="Demo 说明">
                    ${icon("help")}
                </button>
    ...
    function suggestedFlowKey() {
        ...
        while (keys.has(`flow-demo-${index}`)) {
            index++;
        }
        return `flow-demo-${index}`;
    }
    ...
    async function startFlow() {
        ...
        showToast(
            `Execution 已到达稳定状态：${stateLabel(execution.state)}`,
        );
