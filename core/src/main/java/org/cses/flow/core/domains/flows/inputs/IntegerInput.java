...
public class IntegerInput extends Input<Integer> {
     ...
     * @param min 可选的包含下界
     * @param max 可选的包含上界
     * @return 已完成定义校验的 IntegerInput
     * @throws IllegalArgumentException 当字段类型或完整定义非法时抛出
     */
    @JsonCreator
    public static IntegerInput from(
        ...
        return new IntegerInput(
            ...
            defaultValue == null ? null : (Integer) DataType.INTEGER.normalize(defaultValue),
            min, max
        );
    }
    ...
    /**
     * 将非空传输值转换为 Integer，拒绝类型不兼容或越界的值。
    ...
    protected Integer convert(Object value) {
        return (Integer) DataType.INTEGER.normalize(value);
    }
